...
#do.call(rbind, lapply(f_names, function(x) cbind(filter(select(read.dbc(x), c("AM_PESO", "AP_SEXO", "AP_PRIPAL", "AP_CIDPRI")), AP_CIDPRI == "G35"), name= strsplit(x, "\\.")[[1]][[1]])))


dfunct <- function(f_names) {do.call(rbind, lapply(f_names, function(x) cbind(filter(select(read.dbc(x), c("AM_PESO", "AP_SEXO", "AP_PRIPAL", "AP_CIDPRI")), AP_CIDPRI == "G35"), name= strsplit(x, "\\.")[[1]][[1]])))}
...
system.time(f_names[1:15] %>% future_map(dfunct,.options = furrr_options(seed = TRUE)) %>% bind_rows())
system.time(f_names[1:15] %>% dfunct())

df <- f_names %>% future_map(dfunct,.options = furrr_options(seed = TRUE)) %>% bind_rows()
